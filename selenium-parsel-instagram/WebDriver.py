...
class WebDriver:
    ...
    def wait_spinner(self, spinner_selector='article [data-visualcompletion="loading-state"], article [role="progressbar"]'):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Erro inesperado em wait_spinner ao tentar encontrar o spinner: \n{e}")
                return False
            ...
            try:
                self.driver.execute_script(
                    "arguments[0].scrollIntoView(true);", spinner)
            except:
                pass
    ...
    def wait_reply_spinner(self, spinner_index):
        ...
        try:
            ...
            if spinner_index == -1:  # espera rapidamente cada spinner
                ...
                try:
                    self.driver.execute_script(
                        "arguments[0].scrollIntoView(true);", spinners[spinner_index])
                except:
                    pass
    ...
    def login(self, email, senha):
        ...
        try:
            login_button = self.wait.until(EC.element_to_be_clickable((
                By.XPATH, "//div[text()='Entrar']|//div[text()='Log in']"
            )))
            ...
            while not self.loading():  # apenas prossegue quando o carregamento finaliza corretamente(retorna True)
                pass
            login_error = self.driver.find_elements(
                # TODO: css ingles
                By.XPATH, "//div[contains(text(), 'Sua senha está incorreta')] | //div[contains(text(), 'Houve um problema ao entrar no Instagram')]")
            if login_error:
    ...
    def not_now(self):
        not_now_button = self.wait.until(EC.element_to_be_clickable((
            By.XPATH, '//div[text()="Agora não"]|//div[text()="Not now"]'
        )))
    ...
    def search_tag(self, tag, tag_index=0):
        ...
        try:
            search_button = self.wait.until(EC.element_to_be_clickable((
                By.CSS_SELECTOR, 'a svg[aria-label="Pesquisa"], a svg[aria-label="Search"]'
            )))
            search_button.click()  # clica no botao de pesquisa

            search_input = self.wait.until(EC.element_to_be_clickable((
                By.XPATH, '//input[@aria-label="Entrada da pesquisa" and not(ancestor::main)]|//input[@aria-label="Search input" and not(ancestor::main)]'
            )))
            ...
            tag_element.click()
            # salva a tag para garantir que não será raspada novamente
            self.tag_saver.save_tag(tag_text)
        ...
        while True:
            try:
                more_comments_button = self.wait.until(EC.presence_of_element_located((
                    By.XPATH, '//button[.//*[name()="svg"][@aria-label="Carregar mais comentários"]]|//button[.//*[name()="svg"][@aria-label="Load more comments"]]'
                )))
    ...
    def open_hidden_comments(self):
        hidden_comments_button = self.driver.find_elements(
            By.CSS_SELECTOR, 'svg[aria-label="Ver comentários ocultos"] + div span, svg[aria-label="View hidden comments"] + div span')
        ...
        if hidden_comments_button:
            try:
                ...
                try:
                    ...
                    self.driver.execute_script(
                        "arguments[0].click();", hidden_comments_button)
                    self.loading()
            except:
        ...
        try:
            try:
                see_replies_buttons = self.wait.until(EC.presence_of_all_elements_located((
                    By.XPATH, '//button[span[contains(., "Ver respostas")]]|//button[span[contains(., "View replies")]]'
                )))
            except TimeoutException:
                ...
                print(
                    f"Erro inesperado em open_replies ao coletar botões de ver respostas: \n{e}")
                return
            ...
            # espera todas as respostas carregarem
            for num_buttons in range(len(see_replies_buttons)):

                previous_count = len(self.driver.find_elements(
                    By.CSS_SELECTOR, 'ul li h3 + div > span'))
                ...
                if loading_result is True:
                    try:
                        self.wait.until(lambda d: len(d.find_elements(
                            By.CSS_SELECTOR, 'ul li h3 + div > span')) > previous_count)
    ...
    def open_all_comments(self):
        ...
        if self.have_comments():
            ...
            if self.open_more_comments() is True:
                ...
                # só é possível abrir comentários ocultos quando todos os não ocultos forem abertos
    ...
    def save_post_html(post_html):
        ...
        with open("post.html", "wb") as f:
            f.write(etree.tostring(post_html_tree,
                    pretty_print=True, encoding="utf-8"))

    def go_to_next_post(self):
        try:
            next_post_button = self.wait.until(EC.element_to_be_clickable((
                # encontra o filho do botão
                By.XPATH, '//button[.//*[name()="svg"][@aria-label="Avançar"]]|//button[.//*[name()="svg"][@aria-label="Next"]]'
            )))
